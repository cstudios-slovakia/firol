...
export function usePhotoStaging(
    ...
    /**
     * Scopes staging to one nedostatok (Požiarna kniha) rather than the whole
     * item — omitted by every other caller, which keeps matching photos whose
     * `defect_key` is null (today's item-level behaviour, unchanged).
     */
    defectKey?: string,
): PhotoStaging {
    const scoped = useMemo(
        () => (initialPhotos ?? []).filter((p) => (p.defect_key ?? null) === (defectKey ?? null)),
        [initialPhotos, defectKey],
    );
    const [existing, setExisting] = useState<InspectionPhoto[]>(scoped);
    ...
    const initialKey = scoped.map((p) => p.id).join(",");
    useEffect(() => {
        setExisting(scoped);
        setRemovedIds([]);
        setStaged((prev) => {
    ...
    const addFiles = useCallback(
        async (files: FileList | File[]) => {
            ...
            for (const file of list) {
                ...
                if (
                    kept.length + staged.length + accepted.length >=
                    MAX_PHOTOS_PER_ITEM
                ) {
                    rejectedForCap += 1;
                    continue;
            ...
            if (rejectedForCap > 0) {
                setError(
                    `K položke je možné pripojiť najviac ${MAX_PHOTOS_PER_ITEM} fotiek.`,
                );
            } else if (failed > 0) {
                setError(
                    failed === list.length
    ...
    const commit = useCallback(
        async (
            ...
            for (const id of removedIds) {
                try {
                    await Inspections.deleteItemPhoto(
                        inspectionId,
                        itemId,
                        id,
                        csrfToken,
            ...
            for (const photo of staged) {
                try {
                    await Inspections.addItemPhoto(
                        inspectionId,
                        itemId,
                        photo.blob,
                        csrfToken,
...
type ItemPhotoFieldProps = {
    ...
    disabled?: boolean;
    /** Overrides the default "attached as a separate appendix" copy. */
    helpText?: string;
};
...
export function ItemPhotoField({
    photos,
    disabled = false,
    helpText = "Fotky sa pripoja ako samostatná príloha na konci protokolu.",
}: ItemPhotoFieldProps) {
    ...
    // Same order as the grid below, so the lightbox arrows step through
    // exactly what's on screen. Pending (not-yet-uploaded) photos have no
    // viewable image, so they're excluded and don't shift other indices.
    const existingNotPending = photos.existing.filter((p) => !p.pending);
    const viewable = [
        ...existingNotPending.map((p) => ({ src: photoSrc(p, "full") })),
        ...photos.staged.map((p) => ({ src: p.previewUrl })),
    ];
    ...
    return (
        <div className="flex flex-col gap-2">
                        ...
                        p.pending ? (
                            ...
                            <PhotoTile
                                ...
                                onOpen={() =>
                                    lightbox.openAt(
                                        viewable,
                                        existingNotPending.indexOf(p),
                                    )
                                }
                                onRemove={
                                    disabled
                                        ? undefined
                                        : () => photos.removeExisting(p.id)
                                }
                            />
                        ),
                    )}
            ...
            {photos.error ? (
                ...
                !disabled && <p className="text-xs text-ink-400">{helpText}</p>
            )}
        </div>
    );
}
...
export function ItemPhotoStrip({
    ...
    photos: InspectionPhoto[] | undefined;
}) {
    const lightbox = usePhotoLightbox();
    if (!photos || photos.length === 0) return null;

    // Pending (outbox-only) photos have no viewable image, so the lightbox
    // gallery skips them — same reasoning as ItemPhotoField. Track their own
    // index separately since it diverges from the full `photos` array as
    // soon as a pending photo appears before a viewable one.
    const notPending = photos.filter((p) => !p.pending);
    const viewable = notPending.map((p) => ({ src: photoSrc(p, "full") }));
                        ...
                        <li key={p.id}>
                            <button
                                ...
                                onClick={() =>
                                    lightbox.openAt(
                                        viewable,
                                        notPending.indexOf(p),
                                    )
                                }
                                aria-label="Zväčšiť fotku"
                                className="block size-12 cursor-zoom-in overflow-hidden rounded-lg border border-ink-200 bg-ink-50 transition-transform duration-200 hover:scale-105"
                            >
                                <img
                                    src={photoSrc(p, "thumb")}
                                    alt=""
