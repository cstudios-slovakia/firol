<?php
/**
 * Núdzové osvetlenie protocol template.
 *
 * @var string $number
...
<div class="brand-bar">
  ...
  <div class="title"><?= $h($brand['name'] ?? 'Firol') ?> · Záznam o kontrole núdzového osvetlenia</div>
  <div class="subtitle">Test funkčnosti a doby svietenia v núdzovom režime</div>
</div>
...
<h2>Zoznam testovaných svietidiel</h2>
<table class="items">
  <thead>
    <tr>
      <th style="width: 4%">#</th>
      <th style="width: 17%">Typ svietidla</th>
      <th style="width: 17%">Výrobca</th>
      <th style="width: 22%">Umiestnenie</th>
      <th style="width: 12%">Doba (min)</th>
  ...
  <tbody>
    ...
    <tr>
      <td><?= $idx + 1 ?></td>
      <td><?= $h($f['luminaire_type']) ?></td>
      <td><?= $h($f['manufacturer']) ?></td>
      <td><?= $h($f['location']) ?></td>
      <td class="num"><?= (int) ($f['duration_min'] ?? 0) ?></td>
