<?php
/**
 * Požiarne uzávery — prevádzková údržba.
 *
 * @var string $number
...
foreach ($items as $idx => $it) {
    ...
    $kindLabel = $kindLabels[$f['kind'] ?? ''] ?? (string) ($f['kind'] ?? '');
    $bulletItems[] = 'Uzáver č. ' . ($idx + 1) . ' (' . $kindLabel . ' ' . ($f['identifier'] ?? '') . ', ' . ($f['location'] ?? '') . ')';
}
?>
<style>
...
<table class="hdr">
  <tr>
    ...
    <td width="43%" class="hdr-right">
      <div class="hdr-title">Záznam o prevádzkovej údržbe požiarnych uzáverov</div>
...
<table class="items">
  <thead>
    <tr>
      ...
      <th style="width:13%">Druh uzáveru</th>
  ...
  <tbody>
    ...
    <tr>
      <td><?= $idx + 1 ?></td>
      <td><?= $h($kindLabels[$f['kind'] ?? ''] ?? '') ?></td>
      <td><?= $h($f['identifier'] ?? null) ?></td>
...
<div class="tbl-note">Pri všetkých požiarnych uzáveroch bola vykonaná vizuálna kontrola celistvosti, kontrola tesnení, skúška funkčnosti, kontrola samozatváračov, mazanie pohyblivých častí, kontrola závesov a zámkov a kontrola označenia a štítkov.</div>
...
<?php if ($bulletItems): ?>
<div class="notes-box">
  <?php foreach ($bulletItems as $label): ?>
  <p>&#9632; <strong><?= $h($label) ?></strong> – Nevyhovuje.</p>
  <?php endforeach ?>
</div>
<?php endif ?>
...
<table class="sig-tbl">
  <tr>
    ...
    <th width="38%"><?= \Firol\Pdf\SignatureBlock::heading('pu_udrzba') ?></th>
  ...
  <tr class="sig-row">
    ...
    <td>
      <?= \Firol\Pdf\SignatureBlock::signCell($handover ?? null, 'pu_udrzba') ?>
    </td>
    <td></td>
  </tr>
</table>
