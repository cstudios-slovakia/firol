<?php
/**
 * Požiarne vodovody — Záznam o vykonanej kontrole požiarnych vodovodov.
 *
 * @var string $number
...
foreach ($items as $idx => $it) {
    ...
    $typeLabel = ($f['type'] ?? 'other') === 'other' ? ($f['type_other'] ?? 'iný') : (string) ($f['type'] ?? '');
    $label = 'Vodovod č. ' . ($idx + 1) . ' (' . $typeLabel . ', ' . ($f['location'] ?? '') . ')';
    $note = !empty($f['defects']) ? (string) $f['defects'] : 'Nevyhovuje';
...
<style>
  ...
  .tbl-note { margin-top: 3pt; padding: 4pt 8pt; background: #eef5ff; border: 1pt solid #c5d8f5; font-size: 8.5pt; color: #1a3a6b; }
...
<table class="hdr">
  <tr>
    ...
    <td width="43%" class="hdr-right">
      <div class="hdr-title">Záznam o vykonanej kontrole požiarnych vodovodov</div>
...
<h2>Zoznam kontrolovaných požiarnych vodovodov</h2>
<table class="items">
  <thead>
    <tr>
      <th style="width:4%">Č.</th>
      <th style="width:8%">Typ</th>
      <th style="width:21%">Umiestnenie</th>
      <th style="width:9%">HS (MPa)</th>
      <th style="width:9%">HD (MPa)</th>
      <th style="width:8%">Q (l/s)</th>
      <th style="width:9%">Počet hadíc</th>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="num"><?= $formatNum($f['hs'] ?? null) ?></td>
      <td class="num"><?= $formatNum($f['hd'] ?? null) ?></td>
      <td class="num"><?= $formatNum($f['q'] ?? null) ?></td>
      <td class="num"><?= (int) ($f['hose_count'] ?? 0) ?></td>
      <td><?= !empty($f['defects']) ? $h($f['defects']) : '—' ?></td>
      <td class="<?= $r === 'vyhovuje' ? 'res-ok' : ($r === 'nevyhovuje' ? 'res-bad' : '') ?>"><?= $r === 'vyhovuje' ? 'Vyhovuje' : ($r === 'nevyhovuje' ? 'Nevyhovuje' : $h($r)) ?></td>
    </tr>
    <?php endforeach ?>
  </tbody>
</table>
<div class="tbl-note">Na všetkých požiarnych vodovodoch bola vykonaná kontrola označenia, dostupnosti, vybavenosti a funkčnosti.</div>

<h2>Súhrn výsledkov</h2>
<table class="stats">
  <tr>
    <td>Kontrolovaných vodovodov<br><span class="num"><?= (int) $stats['total'] ?></span></td>
