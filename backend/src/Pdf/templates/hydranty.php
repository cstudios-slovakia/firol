<?php
/**
 * Hydranty (požiarne hydranty) protocol template.
 * Receives via PdfRenderer::renderHydranty:
 *
 *   string                 $number          e.g. HYD-2026-001
 ...
 *   list<array>            $items           per-hydrant fields with position
 *   array                  $stats           vyhovuje, nevyhovuje, total
 *
 * @var string $number
...
<h2>Zoznam skontrolovaných požiarnych hydrantov</h2>
<table class="items">
  <thead>
    <tr>
      <th style="width: 4%">#</th>
      <th style="width: 11%">Typ</th>
      <th style="width: 22%">Umiestnenie</th>
      <th style="width: 7%">Hadíc</th>
      <th style="width: 9%">HS&nbsp;(MPa)</th>
      <th style="width: 9%">HD&nbsp;(MPa)</th>
      <th style="width: 9%">Q&nbsp;(l/s)</th>
      <th style="width: 17%">Závady</th>
      <th style="width: 12%">Výsledok</th>
    </tr>
  </thead>
  <tbody>
    <?php foreach ($items as $idx => $it): $f = $it['fields'];
      $typeLabel = $f['type'] === 'other' ? ($f['type_other'] ?? 'iný') : (string) ($f['type'] ?? '');
    ...
    <tr>
      ...
      <td class="notes"><?= !empty($f['defects']) ? nl2br($h($f['defects'])) : '—' ?></td>
      <td><span class="result-pill result-<?= $h($result) ?>"><?= $h($resultLabels[$result] ?? $result) ?></span></td>
    </tr>
    <?php endforeach ?>
  </tbody>
</table>
