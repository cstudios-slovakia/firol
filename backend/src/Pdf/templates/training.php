...
<style>
  ...
  table.topics td.tnum {
    width: 6%;
    text-align: center;
    white-space: nowrap;
  }

  table.topics td.ttime {
    width: 12%;
    text-align: center;
    white-space: nowrap;
  }
  ...
  table.topics-part td {
    padding: 4pt 5pt;
    border: 1pt solid #e5e5ea;
    vertical-align: top;
  }

  table.topics-part td.tnum {
    width: 6%;
    text-align: center;
    white-space: nowrap;
  }
  ...
  table.attendees td.sig-cell {
    text-align: center;
  }

  table.attendees .sig-img {
    max-height: 36pt;
    max-width: 140pt;
  }
...
<table class="hdr">
  <tr>
    ...
    <td width="43%" class="hdr-right">
      <div class="hdr-title">
        <?= in_array($type, ['hliadka_oph', 'hliadka_opah'], true) ? 'Odborná príprava' : 'Školenie PO' ?></div>
...
<table class="bi">
  ...
  <tr>
    ...
    <td class="bv"><?= $formatDate($training['date'] ?? null) ?></td>
  </tr>
  <tr>
    <td class="bl">Prevádzka</td>
    <td class="bv">
      <?= $h($facility['name']) ?><?= !empty($facility['address']) ? '<br><span style="font-weight:normal;color:#555;">' . $h($facility['address']) . '</span>' : '' ?>
    </td>
...
<?php elseif (isset($twoPartTopics[$type])):
  $parts = $twoPartTopics[$type]; ?>
    ...
    <tr>
      ...
      <td style="width:40%; text-align:right; vertical-align:bottom; padding:0;">
        ...
        <?php if (!empty($trainer['signature_data_uri'])): ?>
          <br><img style="max-height:28pt; max-width:110pt; vertical-align:bottom;"
            src="<?= $h($trainer['signature_data_uri']) ?>" alt="">
        <?php else: ?>
