<?php
/**
 * Protokol o vykonanej tlakovej skúške požiarnej hadice.
 *
 * @var string $number
...
<table class="hdr">
  <tr>
    ...
    <td width="43%" class="hdr-right">
      <div class="hdr-title">Protokol o vykonanej tlakovej skúške požiarnej hadice</div>
    ...
    <?php foreach ($items as $idx => $it):
      ...
      <tr>
        ...
        <td class="<?= $r === 'vyhovuje' ? 'res-ok' : ($r === 'nevyhovuje' ? 'res-bad' : '') ?>">
          <?= $r === 'vyhovuje' ? 'Funkčná' : ($r === 'nevyhovuje' ? 'Nefunkčná' : $h($r)) ?></td>
...
<table class="sig-tbl">
  <tr>
    <th width="38%">Skúšku vykonal</th>
    <th width="38%"><?= \Firol\Pdf\SignatureBlock::heading('ts_hadic') ?></th>
  ...
  <tr class="sig-row">
    ...
    <td>
      <?= \Firol\Pdf\SignatureBlock::signCell($handover ?? null, 'ts_hadic') ?>
    </td>
    <td></td>
  </tr>
</table>
